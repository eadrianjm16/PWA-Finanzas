...
def list_categories(db: Session = Depends(get_db_session)) -> list[models.Category]:
    return db.query(models.Category).order_by(models.Category.sort_order).all()


@router.post("", response_model=schemas.CategoryOut, status_code=status.HTTP_201_CREATED)
def create_category(payload: schemas.CategoryCreateRequest, db: Session = Depends(get_db_session)) -> models.Category:
    name = payload.name.strip()
    if not name:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "El nombre no puede estar vacío")
    if db.query(models.Category).filter_by(name=name).first() is not None:
    ...
    category = models.Category(name=name, system_icon_name=payload.system_icon_name, sort_order=max_order)
    db.add(category)
    db.commit()
    db.refresh(category)
    return category
...
def update_category(
    category_id: str, payload: schemas.CategoryUpdateRequest, db: Session = Depends(get_db_session)
) -> models.Category:
    ...
    if payload.name is not None:
        ...
        if not name:
            raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "El nombre no puede estar vacío")
        if name != category.name and db.query(models.Category).filter_by(name=name).first() is not None:
            raise HTTPException(status.HTTP_409_CONFLICT, "Ya existe una categoría con ese nombre")
        category.name = name
...
def delete_category(category_id: str, db: Session = Depends(get_db_session)) -> None:
    category = db.get(models.Category, category_id)
    if category is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Categoría no encontrada")
    ...
    otros = db.query(models.Category).filter_by(name=OTROS_NAME).first()
...
def reorder_categories(
    payload: schemas.CategoryReorderRequest, db: Session = Depends(get_db_session)
