...
def _recurring_candidates(db: Session, user_id: str) -> list[models.Transaction]:
...
def detect_recurring_charges(db: Session, user_id: str) -> list[dict]:
    transactions = _recurring_candidates(db, user_id)
    ...
    for (_, amount_key, currency), group in groups.items():
        ...
        results.append(
            {
                "id": f"sub-{last.account_uid}-{_normalized_key(display_name)}-{amount_key}",
                "name": display_name,
